...
for epoch in xrange(num_epochs):
    print epoch
    progress_filename_train = '../CNN/batch350/log_files/train/drug_like_train-' + str(epoch) + neural_net_present + '_' + test_type + '.csv'
    progress_filename_val = '../CNN/batch350/log_files/val/drug_like_val-' + str(epoch) + neural_net_present + '_' + test_type  + '.csv'
...
progress_filename_test = '../CNN/batch350/log_files/test/drug_like_test-' + neural_net_present + '_' + test_type  + '.csv'
...
tcm_expr_filename = '../CNN/database/ChemDiv.csv'
tcm_fingerprint_filename = '../CNN/fingerprint/ChemDiv_fingerprints.pkl'
smiles_to_measurement,smiles_to_atom_info,smiles_to_bond_info,\
    smiles_to_atom_neighbors,smiles_to_bond_neighbors,smiles_to_atom_mask,\
    smiles_to_rdkit_list,max_atom_len,max_bond_len,num_atom_features,num_bond_features\
...
progress_filename_test = '../CNN/screen_result/drug_like_chemdiv_screen-' + neural_net_present + '_' + test_type  + '.csv'
output_test_df = pd.DataFrame()
for counter, experiment_list in enumerate(expr_list_of_lists):
    temp_test_df = pd.DataFrame()
    # print counter, experiment_list
...
np.savez('../deep_learn/cnn/model.npz', *lasagne.layers.get_all_param_values(cnn_model['output']))
